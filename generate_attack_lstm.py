...
class PGDAdversarialAttacker(AdversarialAttacker):
    ...
    def zero_pad_single(self, audio_embeddings: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Adapted version of zero_pad for single input, used in adversarial attack.
        """
        num_windows = audio_embeddings.shape[0]
        # In adversarial attack, we typically don't have a batch, so no need to get max_num_windows
        return audio_embeddings.unsqueeze(0), torch.tensor([num_windows]).to(audio_embeddings.device)
    ...
    def __call__(
        ...
        for iter_idx in range(self.max_iter):
            ...
            # print(audio_windows_embeddings.shape)
            # Apply zero padding to the embeddings and get batch_num_windows
            audio_windows_embeddings_padded, batch_num_windows = self.zero_pad_single(audio_windows_embeddings)


            y_pred = self.classifier(audio_windows_embeddings_padded, batch_num_windows)
            # Reshape target_label_tensor to match y_pred
            target_label_tensor = target_label_tensor.view_as(y_pred)
...
def is_adversarial_experiment_covered_check(adversarial_experiment_params: AdversarialExperimentParams) -> bool:
    ...
    adversarial_experiments_files_names.remove(".DS_Store")
    print("No of adversarial experiements: ", len(adversarial_experiments_files_names), adversarial_experiments_files_names)
...
def main() -> None:
    ...
    classifier = DeepLSTMModel(input_size=512)
    classifier.load_state_dict(torch.load('classifier_checkpoints/deeplstm_v1.pt', map_location=torch.device('cpu'), weights_only=True))
